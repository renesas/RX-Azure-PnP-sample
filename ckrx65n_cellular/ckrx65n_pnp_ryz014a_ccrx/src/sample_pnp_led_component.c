...
static const CHAR sample_pnp_led_led6_property_name[] = "led6";

static int32_t led2_status;
static int32_t led3_status;
static int32_t led4_status;
static int32_t led6_status;
...
int32_t led6_value;
    ...
    //LOG_TERMINAL("RX_message In--sample_pnp_led_desired_properties_process");
    while (1)
    {
                                                            ...
                                                            sizeof(sample_pnp_led_led3_property_name) - 1))
        {
            ...
            /* Update LED3 property with led3_value. GREEN*/
            switch (led3_status)
            {
            case 1:
        ...
        /* led6 property.  */
                                                                ...
                                                                (UCHAR *)sample_pnp_led_led6_property_name,
                                                                sizeof(sample_pnp_led_led6_property_name) - 1))
        {
            if ((status = nx_azure_iot_json_reader_next_token(json_reader)) ||
                (status = nx_azure_iot_json_reader_token_int32_get(json_reader, &led6_value)))
            {
                return(status);
            }
            ...
            /* Update the state.  */
            led6_status = led6_value;

            /* Update LED6 property with led6_value.  RED*/
            switch (led6_status)
            {
            case 1:
            ...
            sample_pnp_led_reported_properties_send(pnp_client_ptr,
                                                    (UCHAR *)sample_pnp_led_led6_property_name,
                                                    sizeof(sample_pnp_led_led6_property_name) - 1,
                                                    led6_status, SAMPLE_COMMAND_SUCCESS_STATUS, version, "");
        }
        /* led4 property.  */
		else if (nx_azure_iot_json_reader_token_is_text_equal(json_reader,
																...
																sizeof(sample_pnp_led_led4_property_name) - 1))
		{
			...
			/* Update LED4 property with led4_value. BLUE  */
			switch (led4_status)
			{
			case 1:
																...
																sizeof(sample_pnp_led_led2_property_name) - 1))
		{
			...
			/* Update LED2 property with led2_value. RGB */
			switch (led2_status)
			{
			case 1:
