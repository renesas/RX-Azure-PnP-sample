...
UINT system_component;
         ...
         (message_type != NX_AZURE_IOT_HUB_PROPERTIES)))
    {
        ...
        demo_printf("Invalid response type or property type passed\r\n");
        return(NX_AZURE_IOT_INVALID_PARAMETER);
    }

    component_name = az_span_create((UCHAR *)component_name_pptr, (INT)*component_name_length_ptr);
    demo_printf("component_name %s \r\n",component_name._internal.ptr);
    ...
    do
    {
        ...
        else if (az_result_failed(core_result))
        {
            ...
            demo_printf("Failed to parse document with core error : %d", core_result);
            return(NX_AZURE_IOT_SDK_CORE_ERROR);
        }
...
UINT status;
        ...
        (component_name_length_ptr == NX_NULL))
    {
        ...
        demo_printf("IoTHub client component next property failed: INVALID POINTER");

        return(NX_AZURE_IOT_INVALID_PARAMETER);
    }
    else
    {
    	demo_printf("IoTHub client component next property success: VALID POINTER\r\n");
    }
    ...
    demo_printf("nx_azure_iot_hub_client_properties_component_property_next_get_internal %d\r\n",status);
