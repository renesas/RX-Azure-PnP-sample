...
#include "platform.h"
#include "CommandLine/common_init.h"
...
#define SAMPLE_IP_STACK_SIZE          (2098)//(2048)
...
//    if (status)
//    {
//        demo_printf("nx_packet_pool_create fail: %u\r\n", status);
//        return;
//    }
//
...
//    if (status)
//    {
//        demo_printf("nx_arp_enable fail: %u\r\n", status);
//        return;
//    }
//
//    /* Enable ICMP traffic.  */
...
//    if (status)
//    {
//        demo_printf("nx_icmp_enable fail: %u\r\n", status);
//        return;
//    }
//
//    /* Enable TCP traffic.  */
...
//    if (status)
//    {
//        demo_printf("nx_tcp_enable fail: %u\r\n", status);
//        return;
//    }
//
//    /* Enable UDP traffic.  */
...
//    if (status)
//    {
//        demo_printf("nx_udp_enable fail: %u\r\n", status);
//        return;
//    }
//
//    /* Initialize TLS.  */
...
ULONG   physical_lsw;
    ...
    demo_printf("IP address: %lu.%lu.%lu.%lu\r\n",
           (ip_address >> 24),
           (ip_address >> 16 & 0xFF),
           (ip_address >> 8 & 0xFF),
           (ip_address & 0xFF));
    memset(ipaddress_display, 0x00, sizeof(ipaddress_display));
	sprintf(ipaddress_display,"%lu.%lu.%lu.%lu",
	           (ip_address >> 24),
	           (ip_address >> 16 & 0xFF),
	           (ip_address >> 8 & 0xFF),
	           (ip_address & 0xFF));
    demo_printf("Mask: %lu.%lu.%lu.%lu\r\n",
           (network_mask >> 24),
           (network_mask >> 16 & 0xFF),
           (network_mask >> 8 & 0xFF),
           (network_mask & 0xFF));
    demo_printf("Gateway: %lu.%lu.%lu.%lu\r\n",
           (gateway_address >> 24),
           (gateway_address >> 16 & 0xFF),
           (gateway_address >> 8 & 0xFF),
           (gateway_address & 0xFF));

    nx_ip_interface_physical_address_get(&ip_0, interface_index, &physical_msw, &physical_lsw);

    memset(mac_address_with_bar, 0x00, sizeof(mac_address_with_bar));
    sprintf(mac_address_with_bar, "%02x-%02x-%02x-%02x-%02x-%02x",
            (physical_msw >>  8 & 0xFF),
            (physical_msw & 0xFF),
            (physical_lsw >> 24),
            (physical_lsw >> 16 & 0xFF),
            (physical_lsw >>  8 & 0xFF),
            (physical_lsw & 0xFF));
...
UINT    dns_server_address_size = 12;
    ...
    demo_printf("DNS Server address: %lu.%lu.%lu.%lu\r\n",
